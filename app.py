...
def get_min_temp(results):
    ...
    minimum_temp = results[0]['temp']
    for dictionary in results:
        if(dictionary['temp'] < minimum_temp):
            minimum_temp = dictionary['temp']
    return minimum_temp

def get_max_temp(results):
    """Returns the maximum temp for the given hourly weather objects."""
    maximum_temp = results[0]['temp']
    for dictionary in results:
        if(dictionary['temp'] > maximum_temp):
            maximum_temp = dictionary['temp']
    return maximum_temp
...
def historical_results():
    ...
    city = city = request.args.get('city')
